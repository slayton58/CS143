...
void emit_comp_begin(Expression e1, Expression e2, Environment* env) {
  ostream &s = env->str;
...
void emit_comp_end( int label, Environment* env) {
   ostream &s = env->str;
 ...
 // cout << "come out" <<endl;
  ...
  for(int i =0; i < cur_tag; i++) 
  {
    ...
    for (iter = nds.begin(); iter!= nds.end(); ++iter)
    {
      if(i == (*iter)->get_tag()) 
      {
        str << WORD << (*iter)->get_name() << PROTOBJ_SUFFIX << endl;
        str << WORD << (*iter)->get_name() << CLASSINIT_SUFFIX << endl;
      }
    }
  }
}
     ...
     // cout << "@@@@@@@@@@@@@@@" << (*iter_node)->name->get_string() << " method map size: " << (*iter_node)->class_method_map.size() << endl;
      ...
      for(iter_method = (*iter_node)->method_list.begin(); iter_method != (*iter_node)->method_list.end(); ++iter_method) {
        ...
        // method_class* mc = (*iter_method)->method_name;          
         str << (*iter_method)->method_name->name->get_string() << endl;
      }
   }
}
